    ...
    if($("#container-video-iframe").length>0) {    
        $('.element-video-card-iframe .video-img').click(function() {
            ...
            if((url.indexOf('youtu.be') + 1) || (url.indexOf('youtube.com/watch') + 1) ) {
                url = url.replace(new RegExp("index.html", "i"), '');
                url = url.replace(new RegExp("index.html", "i"), '');
                url = url.replace(new RegExp("#\\/\\/youtu.be\\/", "i"), 'http://www.youtube.com/v/');
                url = url.replace(new RegExp("watch\\?v=", "i"), 'v/index.html');
            }
...
function setCover(hardCover) {
    if (!$('#bxr-bprop-table [data-code="HARD_COVER"]').length)
        return;

    currentCardBoard = $('#bxr-bprop-table [data-code="CARDBOARD_COVER"]').prop("checked");

    switch (hardCover) {
        case 0:
            $('#bxr-bprop-table [data-code="HARD_COVER"]').attr("disabled", "disabled");
            $('#bxr-bprop-table [data-code="HARD_COVER"]').prop("checked", "");
            $('#bxr-bprop-table [data-code="CARDBOARD_COVER"]').removeAttr("disabled");
            break;
        case 1:
            $('#bxr-bprop-table [data-code="HARD_COVER"]').attr("disabled", "disabled");
            $('#bxr-bprop-table [data-code="HARD_COVER"]').prop("checked", "checked");
            $('#bxr-bprop-table [data-code="CARDBOARD_COVER"]').attr("disabled", "disabled");
            $('#bxr-bprop-table [data-code="CARDBOARD_COVER"]').prop("checked", "");
            break;
        case 2:
            curCardState = $('#bxr-bprop-table [data-code="CARDBOARD_COVER"]').prop("checked");
            if (curCardState == true)
                $('#bxr-bprop-table [data-code="HARD_COVER"]').attr("disabled", "disabled");
            else
                $('#bxr-bprop-table [data-code="HARD_COVER"]').removeAttr("disabled");
            $('#bxr-bprop-table [data-code="HARD_COVER"]').prop("checked", "");
            $('#bxr-bprop-table [data-code="CARDBOARD_COVER"]').removeAttr("disabled");
            break;
    }
...
function getCurrentBasketProps() {
    bProps = [];
    $('.bxr-bprop-select').each(function() {
        bProps.push($(this).val());
    });

    $('.bxr-bprop-chb:checked').each(function() {
        bProps.push($(this).val());
    });
...
function getNewPrice() {
    BXReady.showAjaxShadow("#bxr-market-buy-block", "detail-price-shadow");

    itemId = $('#bxr-market-detail-basket-btn-wrap .bxr-quantity-text').data("item");
    qty = $('#bxr-market-detail-basket-btn-wrap .bxr-quantity-text').val();
    bProps = getCurrentBasketProps();
    ...
    $.ajax({
        url: '/ajax/getPriceItem.php',
        type: "POST",
    	data: params,
        success: function(data) {
            data = JSON.parse(data);
            console.log(data);
    	    if ($('#bxr-market-price-wrap .bxr-market-current-price').length) {
                $('#bxr-market-price-wrap .bxr-market-current-price').html(data.PRICES.DISCOUNT_PRICE_FORMATTED);
            }
            if ($('#bxr-market-price-wrap .bxr-market-old-price').length) {
                $('#bxr-market-price-wrap .bxr-market-old-price').html(data.PRICES.PRICE.PRICE_FORMATTED);
            }
    	    BXReady.closeAjaxShadow("detail-price-shadow");
    	},
    });
}

$(document).on("change", ".bxr-bprop-select", function() {
    value = $(this).val();
    hardCover = $(this).find('option:selected').data("hardcover");
    setCover(hardCover);
    getNewPrice();
});

$(document).on("change", ".bxr-bprop-chb", function() {
    code = $(this).data("code");

    if (code == "CARDBOARD_COVER") {
        currentState = $(this).prop("checked");
        lastHardCoverD = $('#bxr-bprop-table [data-code="HARD_COVER"]').attr("disabled");

        if (lastHardCoverD == undefined && currentState == true) {
            $('#bxr-bprop-table').data("hardcoverlasts", "on");
            setCover(0);
        }

        if ($('#bxr-bprop-table').data("hardcoverlasts") == "on" && currentState != true) {
            $('#bxr-bprop-table').data("hardcoverlasts", "");
            setCover(2);
        }
    } else if (code == "HARD_COVER") {
        currentState = $(this).prop("checked");
        if (currentState) {
            $('#bxr-bprop-table [data-code="CARDBOARD_COVER"]').attr("disabled", "disabled");
            $('#bxr-bprop-table [data-code="CARDBOARD_COVER"]').prop("checked", "");
            ...
            $('#bxr-bprop-table [data-code="CARDBOARD_COVER"]').removeAttr("disabled");
    } else if (code == "NUMBERING") {
        currentState = $(this).prop("checked");
        if (currentState) {
            $('#bxr-bprop-table [data-code="OFFSET_PAPER"]').attr("disabled", "disabled");
            $('#bxr-bprop-table [data-code="OFFSET_PAPER"]').prop("checked", "checked");
        } else
            $('#bxr-bprop-table [data-code="OFFSET_PAPER"]').removeAttr("disabled");
    };

    getNewPrice();
});

$(document).on("change", "#bxr-market-detail-basket-btn-wrap .bxr-quantity-text", function() {
    getNewPrice();
});
